...
public final strictfp class Transform implements Serializable {
    ...
    public static Transform combine(Transform t1, Transform t2) {
        float[] a = t1.data;
        float[] b = t2.data;
        return new Transform(a[0] * b[0] + a[4] * b[1] + a[12] * b[3],
    ...
    public static Transform translate(Transform t, float x, float y) {
        return combine(t, new Transform(1, 0, x,
                                        0, 1, y,
                                        0, 0, 1));
    }
    ...
    public static Transform rotate(Transform t, float angle) {
        final float rad = MathUtils.degToRad(angle), cos = (float) Math.cos(rad), sin = (float) Math.sin(rad);
        return combine(t, new Transform(cos, -sin, 0,
                                        sin, cos, 0,
                                        0, 0, 1));
    }
    ...
    public static Transform rotate(Transform t, float angle, float centerX, float centerY) {
        final float rad = MathUtils.degToRad(angle), cos = (float) Math.cos(rad), sin = (float) Math.sin(rad);
        return combine(t, new Transform(cos, -sin, centerX * (1 - cos) + centerY * sin,
    ...
    public static Transform scale(Transform t, float scaleX, float scaleY) {
        return combine(t, new Transform(scaleX, 0, 0,
                                        0, scaleY, 0,
                                        0, 0, 1));
    }
     ...
     *
     * @return the inverse transformation, or the identity transformation if the matrix cannot be inverted
     */
    public Transform getInverse() {
        final float det = data[0] * (data[15] * data[5] - data[7] * data[13]) -
                          data[1] * (data[15] * data[4] - data[7] * data[12]) +
                          data[3] * (data[13] * data[4] - data[5] * data[12]);
        if (det != 0.f) return new Transform((data[15] * data[5] - data[7] * data[13]) / det,
    ...
    public Vector2f transformPoint(float x, float y) {
        return new Vector2f(data[0] * x + data[4] * y + data[12],
    ...
    public boolean equals(Object o) {
        return (o instanceof final Transform t && Arrays.equals(data, t.data));
    }
    ...
    public String toString() {
        ...
        int i = 0;
        for (int y = 0; y < 4; y++) {
            for (int x = 0; x < 4; x++) str.append(data[i++]).append(" ");
            str.append("\n");
        }
        str.append("}");
        return str.toString();
